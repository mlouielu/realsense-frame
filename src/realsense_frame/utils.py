...
def create_colorbar(height, width=20, cmap=cv2.COLORMAP_JET, max_depth_meters=8.5, alpha=0.03):
    ...
    cv2.putText(colorbar, max_label, (5, 15), font, font_scale, text_color, font_thickness, cv2.LINE_AA)

    # Min depth label (0m)
    min_label = "0m"
    text_size = cv2.getTextSize(min_label, font, font_scale, font_thickness)[0]
    cv2.putText(colorbar, min_label, (5, height - text_size[1] - 5), font, font_scale, text_color, font_thickness, cv2.LINE_AA)
